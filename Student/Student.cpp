...
void Student::load_data(ifstream &fin, List<Course*> &pAllCourses) {
...
void Student::save_data(ofstream &fout) {
    // format
    // ID
    // class_name
    // fClass[8][5]
    ...
    for (Node<CourseScore>* p = CoursesList.begin(); p != NULL; p = p -> pNext) {
